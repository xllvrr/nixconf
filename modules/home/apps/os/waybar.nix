...
{
  options = {
    xllvr.desktop.waybar.enable = lib.mkEnableOption "Waybar";
  };

  config = lib.mkIf config.xllvr.desktop.waybar.enable {
    xdg.configFile =
      let
    ...
    programs.waybar = {
      ...
      style =
        ...
        ''
        + builtins.readFile (repoRoot + "/configs/waybar/style.css");
    };
  };
}
